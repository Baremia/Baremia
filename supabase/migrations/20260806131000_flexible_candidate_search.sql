create or replace function baremia.tokens_nombre(p_text text)
returns text[]
language sql
immutable
set search_path = ''
...
create or replace function baremia.firma_nombre(p_text text)
returns text
language sql
immutable
set search_path = ''
as $$
  select array_to_string(baremia.tokens_nombre(p_text), ' ');
$$;
